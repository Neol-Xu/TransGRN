...
def stratified_tf_split(tf_network, test_size=0.4, val_size=0.5, random_seed=42):
    # Stratify TFs based on the number of target genes
    tiers = {
        'tier1': [],  # >500
        'tier2': [],  # 100-500
        'tier3': [],  # 10-100
    ...
    # Split each tier (tier1-3 by ratio, tier4 all to test set)
...
def generate_edges(tf_network, selected_tfs, all_genes, used_genes, used_targets, mode='train', density=None, random_seed=42):
    ...
    for tf in selected_tfs:
        ...
        # Determine number of negative samples based on mode
        if mode in ['train', 'val']:
            n_neg = len(available_targets)  # 1:1
        else:
            n_pos = len(available_targets) # density-based
...
def create_dataset(label_file, Gene_file, TF_file, output_dir, 
    ...
    for attempt in range(max_retry):
        ...
        assert len(train_tg & val_tg) == 0, "Train and Val sets have overlapping Targets"
        assert len(train_tg & test_tg) == 0, "Train and Test sets have overlapping Targets"
        assert len(val_tg & test_tg) == 0, "Val and Test sets have overlapping Targets"
        
        print(f"[INFO] Sampling succeeded, number of attempts: {attempt + 1}")

        return stats
    raise RuntimeError("[ERROR] Unable to generate non-empty train/validation/test sets within the specified maximum number of attempts")
    ...
    for net_type in net_types:
        for num in nums:
            for data_type in data_types:
                base_dir = os.getcwd()
                TF_file = os.path.join(base_dir, net_type, data_type, f'TFs+{num}', 'TF.csv')
                Gene_file = os.path.join(base_dir, net_type, data_type, f'TFs+{num}', 'Target.csv')
